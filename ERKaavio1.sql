...
CREATE TABLE tuoteryhma (
                ...
                CONSTRAINT tuotaryhma_pk PRIMARY KEY (tuoteryhma)
);


CREATE TABLE tuote (
                tyokalukoodi INTEGER NOT NULL,
                tuoteryhma VARCHAR(20) NOT NULL,
                nimike VARCHAR(30) NOT NULL,
                merkki VARCHAR(25) NOT NULL,
                malli VARCHAR(25) NOT NULL,
                paivavuokra FLOAT NOT NULL,
                CONSTRAINT tuote_pk PRIMARY KEY (tyokalukoodi)
);
...
COMMENT ON COLUMN yhteystyyppi.yhteystyyppi IS 'Taulu on luotu vain käyttöliittymää varten. Se yestää käyttäjää syöttämästä Yhteyshenkilö-tauluun muita kuin ennalta määrättyjä arvoja.';
...
COMMENT ON COLUMN yhteyshenkilo.yhteystyyppi IS 'Taulu on luotu vain käyttöliittymää varten. Se yestää käyttäjää syöttämästä Yhteyshenkilö-tauluun muita kuin ennalta määrättyjä arvoja.';
...
FOREIGN KEY (tyokalukoodi)
REFERENCES tuote (tyokalukoodi)
ON DELETE NO ACTION
ON UPDATE NO ACTION
NOT DEFERRABLE;
